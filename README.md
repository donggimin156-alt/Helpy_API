...
### ✅ 해결됨: Allure Overview의 Behaviors 위젯 미표시

**현상:** Allure 리포트 Overview 및 Behaviors 탭에서 `500 / Unexpected token '<', "<!DOCTYPE"... is not valid JSON` 표시.

**원인:**

- Allure CLI 2.40.0이 새 포맷(`widgets/` 디렉토리)으로 리포트를 생성
- Jenkins Allure 플러그인이 구 포맷 기준으로 `data/behaviors.json`을 요청 → 302(파일 없음) → HTML 반환 → JSON 파싱 오류

**해결:** Jenkins Tools → Allure Commandline 버전을 `2.40.0` → `2.27.0`으로 다운그레이드.

2.27.0은 `data/behaviors.json`을 정상 생성하며, TREND·SUITES·History 기능도 모두 정상 작동한다.
